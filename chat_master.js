...
// Runs at document_start ÔÇö signals presence immediately and adapts behavior based on site
(function () {
  'use strict';

  // ÔöÇÔöÇ Signal presence via DOM attribute ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // ÔöÇÔöÇ Module-level AI state ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  let aiMessages = [];
  let currentUserId = 0;
  let currentSesskey = '';
  ...
  // ÔöÇÔöÇ Shared Moodle config helpers (Only needed on Moodle origin) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // ÔöÇÔöÇ Helper to fetch via Background script (cross-origin bypass) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // ÔöÇÔöÇ Shared Moodle AJAX callers ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // ÔöÇÔöÇ Fuzzy Search Algorithmic utilities ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // ÔöÇÔöÇ Moodle Native searches (direct on moodle origin) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  ...
  // ÔöÇÔöÇ Moodle Deep Search Engine (runs direct on Moodle pages) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  const deepSearchMoodle = async (query) => {
    ...
    for (const c of courseHits.slice(0, 5)) {
      results.push({ type: 'course', icon: '­ƒôÜ', title: cleanCourseTitle(c.fullname), subtitle: c.shortname || c.fullname, url: `${origin}/course/view.php?id=${c.id}`, score: c._score });
    }
    ...
    try {
      ...
      for (const ev of calendarEvents) {
        ...
        if (score >= MATCH_THRESHOLD || (isDevoirsIntent && isAssignEvent) || (isQuizIntent && isQuizEvent) || (isCalendarIntent && !calendarFilterQuery)) {
          ...
          results.push({
            type: 'deadline', icon: '­ƒôà', title: evName,
            subtitle: `${cleanCourseTitle(courseName)} ÔÇó ├Ç rendre : ${dateStr}`,
    ...
    try {
      ...
      if (searchQ) {
        ...
        for (const nr of nativeResults) {
          ...
          results.push({
            type: isFile ? 'file' : 'activity', icon: isFile ? '­ƒôä' : '­ƒôî',
    ...
    const moduleTypeIcon = (mod) => {
      ...
      if (t === 'resource' || t === 'url') return '­ƒôä';
      if (t === 'folder') return '­ƒôü';
      if (t === 'assign') return '­ƒôØ';
      if (t === 'quiz') return 'ÔØô';
      if (t === 'forum') return '­ƒÆ¼';
      if (t === 'page') return '­ƒîÉ';
      if (t === 'scorm' || t === 'h5pactivity') return '­ƒÄ«';
      if (t === 'video' || t === 'videofile') return '­ƒÄÑ';
      if (t === 'glossary') return '­ƒôû';
      if (t === 'wiki') return '­ƒôØ';
      if (t === 'choice') return '­ƒù│´©Å';
      if (t === 'survey') return '­ƒôè';
      if (t === 'lesson') return '­ƒÄô';
      return '­ƒôî';
    };
    ...
    for (const pr of courseContents) {
      ...
      for (const section of (sections || [])) {
        ...
        if ((secScore >= MATCH_THRESHOLD || isScopedSearch) && secName) {
          results.push({ type: 'section', icon: '­ƒôé', title: secName, subtitle: `Section dans : ${cleanCourseTitle(course.fullname)}`, url: `${origin}/course/view.php?id=${course.id}#section-${section.section}`, score: isScopedSearch ? 0.90 + (secScore * 0.09) : secScore });
        }
        ...
        for (const mod of section.modules) {
          ...
          if (modScore >= MATCH_THRESHOLD || (isDevoirsIntent && mod.modname === 'assign') || (isQuizIntent && mod.modname === 'quiz')) {
            results.push({ type: mod.modname || 'module', icon: moduleTypeIcon(mod), title: modName, subtitle: `${cleanCourseTitle(course.fullname)}${secName ? ' ÔÇ║ ' + secName : ''}`, url: mod.url || `${origin}/course/view.php?id=${course.id}`, score: isScopedSearch ? 0.90 + (modScore * 0.09) : modScore });
          }
          ...
          for (const file of mod.contents) {
            ...
            if (fileScore >= MATCH_THRESHOLD || (isFilesIntent && isTargetCourseFile)) {
              ...
              const fileIcon = ext === 'pdf' ? '­ƒôò' : ['doc','docx'].includes(ext) ? '­ƒôÿ' : ['xls','xlsx'].includes(ext) ? '­ƒôù' : ['ppt','pptx'].includes(ext) ? '­ƒôÖ' : ['zip','rar','7z'].includes(ext) ? '­ƒù£´©Å' : ['mp4','avi','mkv','mov'].includes(ext) ? '­ƒÄÑ' : ['mp3','wav','ogg'].includes(ext) ? '­ƒÄÁ' : ['jpg','jpeg','png','gif','svg'].includes(ext) ? '­ƒû╝´©Å' : ['py','js','java','c','cpp','html','css'].includes(ext) ? '­ƒÆ╗' : '­ƒôä';
              results.push({ type: 'file', icon: fileIcon, title: fname, subtitle: `${cleanCourseTitle(course.fullname)} ÔÇ║ ${modName}${secName ? ' ÔÇ║ ' + secName : ''}`, url: file.fileurl || mod.url || `${origin}/course/view.php?id=${course.id}`, score: isScopedSearch ? 0.90 + (fileScore * 0.09) : fileScore, ext });
            }
          }
        }
      }
    ...
    if (!isScopedSearch) {
      try {
        ...
        for (const u of userHits.slice(0, 3)) {
          ...
          results.push({ type: 'user', icon: '­ƒæñ', title: fullname, subtitle: u.email || '', url: `${origin}/user/profile.php?id=${u.id}`, score: fuzzyScore(query, fullname) + 0.1 });
        }
  ...
  // ÔöÇÔöÇ Moodle Result Render HTML ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  const iaRenderResults = (results, query) => {
    if (results.length === 0) return `<div class="ia-no-results">­ƒÿò Je n'ai rien trouv├®, d├®sol├®.</div>`;
    const typeLabel = { course: 'Cours', section: 'Section', resource: 'Ressource', assign: 'Devoir', quiz: 'Quiz', forum: 'Forum', page: 'Page', folder: 'Dossier', url: 'Lien', user: 'Utilisateur', module: 'Activit├®', file: 'Fichier', deadline: 'Date limite' };
    ...
    const summaryParts = [];
    if (otherCount > 0) summaryParts.push(`${otherCount} activit├®${otherCount > 1 ? 's' : ''}`);
    ...
    let html = `<div class="ia-results-header">${results.length} r├®sultat${results.length > 1 ? 's' : ''} pour <strong>"${query}"</strong>${summaryParts.length ? ' <span class="ia-results-breakdown">(' + summaryParts.join(', ') + ')</span>' : ''}</div><div class="ia-results-list">`;
  ...
  const getAdaptedSuggestions = (results, query) => {
    const destinations = [
      { name: "├ëtats-Unis", keywords: ["etats unis","usa","us","united states","irvine","uci","california","californie","los angeles","lax","anteaters"] },
      { name: "Montr├®al", keywords: ["montreal","concordia","yul","hec montreal","hec","mcgill","uqam","vieux montreal","plateau mont royal","guy concordia","loyola campus","sgw campus"] },
  ...
  // ÔöÇÔöÇ MOODLE ORIGIN ENTRYPOINT LOGIC ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
  // ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
  if (isMoodle) {
    const getAiMessages = () => {
      if (aiMessages.length === 0) {
        aiMessages.push({
          useridfrom: 0,
          text: `<div class="ia-welcome"><div class="ia-welcome-icon">­ƒöì</div><strong>Recherche Moodle intelligente</strong><p>Je peux trouver n'importe quel contenu : cours, fichiers, devoirs, quiz, deadlines, calendriers, utilisateursÔÇª m├¬me avec des fautes de frappe !</p><p class="ia-welcome-hint">Essaie par exemple : <em>"deadlines"</em>, <em>"devoir en eco"</em>, <em>"algo"</em></p></div>`,
    ...
    const handleAiChatMessage = async (text) => {
      ...
      try {
        ...
        removeAiTyping(typing);
        aiMessages.push({ useridfrom: 0, text: `ÔØî Une erreur est survenue : <em>${err.message}</em>`, timecreated: Date.now() / 1000 });
        renderAiChat();
      }
    };
  ...
  // ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
  // ÔöÇÔöÇ MYEFREI PORTAL ENTRYPOINT LOGIC (Full Page Chatbot in /racywama/ai) ÔöÇÔöÇ
  // ÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉÔòÉ
  else {
    ...
    // ÔöÇÔöÇ Moodle AJAX callers from MyEfrei origin (delegated via service worker) ÔöÇÔöÇ
    ...
    // ÔöÇÔöÇ Moodle Deep Search Engine (Cross-platform variant) ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    ...
    // ÔöÇÔöÇ Unified Search Engine (MyEfrei database + Moodle cross-platform fallback) ÔöÇÔöÇ
            ...
            ues.forEach(ue => {
              ...
              if (ueScore >= 0.5) {
                ...
                results.push({
                  ...
                  subtitle: `Semestre ${periodObj.period} (${periodObj.schoolYear}) ÔÇó Coef: ${ue.coef || ue.ectsAttempted || 1}`,
                  url: '/portal/student/grades',
                  score: Math.min(ueScore + 0.15, 1.0), // Boosted priority for grades
                  meta: { average: avgVal != null ? `${avgVal}/20` : 'Non not├®' }
                });
              }
                ...
                subjects.forEach(sub => {
                  ...
                  if (subScore >= 0.5) {
                    ...
                    results.push({
                      ...
                      icon: '­ƒôÜ',
                      title: subName,
                      subtitle: `Mati├¿re dans l'UE : ${ueName} (${periodObj.period})`,
                      url: '/portal/student/grades',
                      score: Math.min(subScore + 0.15, 1.0), // Boosted priority for grades
                      meta: { average: average != null ? `${average}/20` : 'Non not├®' }
                    });
                  }
                });
              }
      ...
      if (focusSettings.myefrei && myefreiCache.documents.length > 0) {
        myefreiCache.documents.forEach(doc => {
          ...
          if (score >= 0.5) {
            ...
            results.push({
              type: 'mye-document',
              icon: doc.source === 'invoice' ? '­ƒº¥' : '­ƒôä',
              title: doc.title,
              subtitle: `${doc.category || 'Scolarit├®'} ÔÇó T├®l├®chargement`,
              url: downloadUrl,
        ...
        myefreiCache.contacts.forEach(cat => {
          ...
          contacts.forEach(c => {
            ...
            if (score >= 0.5) {
              results.push({
                type: 'mye-contact',
                icon: c.type === 'staff' ? '­ƒæñ' : '­ƒÅó',
                title: c.title,
                subtitle: `${c.jobTitle || 'Service'} ÔÇó ${catTitle}`,
                url: c.link || '',
            ...
            group.items.forEach(resItem => {
              ...
              if (score >= 0.5) {
                results.push({
                  ...
                  icon: '­ƒôü',
                  title: rTitle,
                  subtitle: `Ressources ÔÇ║ ${cat.category} ÔÇ║ ${group.name}`,
    ...
    // ÔöÇÔöÇ MyEfrei Render results HTML ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
    const myeRenderResults = (results, query) => {
        ...
        if (hasNotesKeyword) {
          ...
          if (bestSubjectMatch) {
            ...
            if (latestPeriod) {
              ...
              let avgStr = generalAverage != null ? `<strong>${Number(generalAverage).toFixed(2)}</strong>` : "non sp├®cifi├®e";
              notesText = `Votre moyenne g├®n├®rale est de ${avgStr}.`;
              ...
              notesText = targetSem ? `Aucune note enregistr├®e pour le semestre ${targetSem}.` : "Vous n'avez pas encore de notes enregistr├®es.";
            }
          }
          ...
          aiSummaryHTML = `
            <div class="mye-ai-summary-card" style="background: linear-gradient(135deg, #eef2ff 0%, #e0e7ff 100%); border: 1.5px solid #c7d2fe; border-radius: 20px; padding: 20px; margin-bottom: 20px; font-family: 'Outfit', sans-serif;">
              <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 8px;">
                <span style="font-size: 20px;">­ƒñû</span>
          ...
          if (deadlines.length > 0) {
            moodleText = `Vous avez <strong>${deadlines.length}</strong> devoir${deadlines.length > 1 ? 's' : ''}/├®ch├®ance${deadlines.length > 1 ? 's' : ''} ├á venir sur Moodle.`;
            moodleText += `<br><br>Voici vos prochaines ├®ch├®ances :`;
            ...
            courses.slice(0, 3).forEach(c => {
              moodleText += `
                <div class="mye-ai-summary-module-row">
                  ...
                  <a href="${c.url}" target="_blank" style="color: #4f46e5; text-decoration: none; font-size: 13px; font-weight: 600;">Acc├®der</a>
                </div>
              `;
            });
            moodleText += `</div>`;
          } else {
            moodleText = "Aucune ├®ch├®ance ou cours Moodle particulier trouv├® pour cette recherche.";
          }
          ...
          if (myefreiCache.documents.length > 0) {
            docsText += `<br><br>Voici vos documents r├®cents :`;
            ...
            myefreiCache.documents.slice(0, 3).forEach(doc => {
              ...
              docsText += `
                <div class="mye-ai-summary-module-row">
                  ...
                  <a href="${downloadUrl}" target="_blank" style="color: #4f46e5; text-decoration: none; font-size: 13px; font-weight: 600;">T├®l├®charger</a>
                </div>
              `;
            });
            docsText += `</div>`;
          ...
          if (contactsList.length > 0) {
            ...
            contactsText = "Aucun contact enregistr├® trouv├®.";
          }
            ...
            resourcesList.forEach(r => {
              resourcesText += `
                <div class="mye-ai-summary-module-row">
                  ...
                  <a href="${r.url}" target="_blank" download style="color: #4f46e5; text-decoration: none; font-size: 13px; font-weight: 600;">T├®l├®charger</a>
                </div>
              `;
            });
            resourcesText += `</div>`;
          } else {
            resourcesText = "Aucune ressource utile enregistr├®e.";
          }
      ...
      if (results.length === 0 && !aiSummaryHTML) {
        return `<div class="ia-no-results">­ƒÿò Je n'ai trouv├® aucun r├®sultat correspondant dans MyEfrei ou Moodle.</div>`;
      }

      const typeLabel = {
        'mye-grade-ue': 'Moyenne UE',
        'mye-grade-subject': 'Mati├¿re',
      ...
      let html = '';
      if (results.length > 0) {
        html += `<div class="ia-results-header" style="font-size: 16px; font-weight: 700; color: #475569; margin-bottom: 16px;">${results.length} r├®sultat${results.length > 1 ? 's' : ''} trouv├®${results.length > 1 ? 's' : ''} pour <strong>"${query}"</strong></div>`;
      }
      ...
      if (results.length > 0) {
        ...
        for (const r of results) {
          ...
          if (r.type === 'mye-grade-ue' || r.type === 'mye-grade-subject') {
            ...
            if (c.phone) {
              actions += `<a href="tel:${c.phone}" class="mye-ai-contact-btn" title="T├®l├®phoner" style="display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 50%; background: #ecfdf5; color: #059669; margin-left: 5px;"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg></a>`;
            }
    ...
    // ÔöÇÔöÇ Helper to setup UI events on Portal chatbot ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
      ...
      if (scrollable) {
        ...
        loadingRow.innerHTML = `
          <div class="mye-chat-bot-header">
            <span class="mye-chat-bot-avatar">­ƒñû</span>
        ...
        botRow.innerHTML = `
          ...
          <div class="mye-chat-bot-content">
            <div style="padding: 20px; text-align: center; color: #ef4444; font-weight: 600; background: rgba(239, 68, 68, 0.05); border-radius: 16px; border: 1px solid rgba(239, 68, 68, 0.1);">
              ÔØî Une erreur est survenue : <em>${msg}</em>
            </div>
          </div>
        `;
        scrollable.appendChild(botRow);
    ...
    // ÔöÇÔöÇ Inject the full page Chat UI container ÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇÔöÇ
            ...
            <div class="mye-ai-center-view">
              <div class="mye-ai-landing" id="mye-ai-landing">
                ...
                <div class="mye-ai-quick-links">
                  <button class="mye-ai-link-btn" data-query="actualit├®s">Actualit├®</button>
                  <button class="mye-ai-link-btn" data-query="mes cours">Mes cours</button>
                  <button class="mye-ai-link-btn" data-query="contacts scolarit├®">Envoyer un DM</button>
                </div>
              </div>
              ...
              <div class="mye-ai-results-panel mye-hidden" id="mye-ai-results-panel">
                <div class="mye-ai-results-header-row">
                  <button id="mye-ai-back-btn" class="mye-ai-back-btn" title="Retour ├á l'accueil">
