(function () {
    ...
    // --- On joue les fous du bus : on prend ce qu'on a, pas d'erreur ---
    const getStoredMoodleConfig = async () => {
        return new Promise((resolve) => {
            if (currentSesskey) {
                return resolve({ sesskey: currentSesskey, userid: currentUserId || 0 });
            }
            if (chrome && chrome.storage && chrome.storage.local) {
                chrome.storage.local.get(['moodle_sesskey', 'moodle_userid'], function(result) {
                    currentSesskey = result.moodle_sesskey || '';
                    currentUserId = result.moodle_userid ? parseInt(result.moodle_userid, 10) : 0;
                    resolve({ sesskey: currentSesskey, userid: currentUserId });
                });
            } else {
                resolve({ sesskey: '', userid: 0 });
            }
        });
    };
    
    const callMoodleAjax = async (methodname, args) => {
        const config = await getStoredMoodleConfig();
        // Même si la sesskey est vide, on tente l'appel. Moodle nous dira si c'est pas bon.
        const sesskey = config.sesskey || '';
        
        const url = `${MOODLE_ORIGIN}/lib/ajax/service.php?sesskey=${sesskey}&info=${methodname}`;
        
        // Comme on est depuis la page d'extension vers moodle, on inclut bien les credentials
        // pour que le navigateur envoie les cookies de session Moodle automatiquement.
    ...
    // On n'attend plus, on renvoie ce qu'on a (même 0)
    const waitForUserId = () => new Promise(async (resolve) => {
        const config = await getStoredMoodleConfig();
        resolve(config.userid || 0);
    });
    ...
    const renderAiChat = () => {
        ...
        getAiMessages().forEach(msg => {
            const isSelf = msg.useridfrom !== 0;
            const messageEl = document.createElement('div');
            messageEl.className = `chat-message ${isSelf ? 'user-message' : 'bot-message'}`;
    ...
    const showAiTyping = () => {
        ...
        typingEl.innerHTML = '<div class="ia-typing"><span></span><span></span><span></span></div>';
        chatMessages.appendChild(typingEl);
        chatMessages.scrollTop = chatMessages.scrollHeight;
        return typingEl;
    };

    const removeAiTyping = (el) => {
        if (el && el.parentNode) {
    ...
    const handleAiChatMessage = async (text) => {
        aiMessages.push({ useridfrom: currentUserId || 1, text, timecreated: Date.now() / 1000 });
        renderAiChat();
